...
export default {
   ...
   * 유효성 검사 결과 메시지를 반환
   * @param {String} value
   * @param {Array, String} trgt
   * @returns {String} msg
   */
   checkValidation(value, trgt) {
    if (!(this.getType(trgt) === "array" || this.getType(trgt) === "string" || this.getType(trgt) === "number")) {
      throw new this.CumtomException('두번째 인자는 Array 또는 String 타입만 가능합니다.');
    }
    if (trgt.indexOf("required") > -1 && this.isEmpty(value)) {
      return "필수 입력 항목입니다";
    } else {
      if (this.isNotNull(value) && value.toString().length > 0) {
        if (trgt.indexOf("korMore2") > -1 && this.getBytes(value) < 4) {
          return "한글 최소 2자이상 입력하세요";
        } else if (trgt.indexOf("more4") > -1 && value.length <= 4) {
          return "4 ~ 10 자리로 입력해주세요";
        } else if (trgt.indexOf("loginId") > -1 && !this.isLoginId(value)) {
          return "영문(대소문자)과 숫자만 입력가능합니다";
        } else if (trgt.indexOf("idttNo") > -1 && !this.isIdttNo(value)) {
          return "주민(사업자)번호가 올바르지 않습니다";
        } else if (trgt.indexOf("mobile") > -1 && !this.isMobile(value)) {
          return "휴대전화번호가 올바르지 않습니다";
        } else if (trgt.indexOf("phone") > -1 && !this.isPhone(value)) {
          return "전화번호가 올바르지 않습니다";
        } else if (trgt.indexOf("onlyNum") > -1 && !this.isNum(value)) {
          return "숫자만 입력가능합니다";
        } else if (trgt.indexOf("email") > -1 && !this.isEmail(value)) {
   ...
   * @returns {Boolean}
   */
  isUserId(userId, permitChar) {
    return (/^[a-zA-Z]{1}[0-9a-zA-Z]+$/).test(this.remove(userId, permitChar)) ? true : false;
  },

  /**
   * 아이디 체크: 영어와 숫자만 체크 - args[0] : 아이디 값, args[1] : 추가 허용할 문자들
   * @param {String} userId
   * @param {String} permitChar
   * @returns {Boolean}
   */
  isLoginId(userId, permitChar) {
    return (/^[0-9a-zA-Z]+$/).test(this.remove(userId, permitChar)) ? true : false;
  },
  ...
  /**
   * 사업자번호 체크 - args[0] : 등록번호 , args[1] : 등록번호 구분자
   * 000-00-00000
   * @param {String} idttNo
   * @param {String} separator
   * @returns {Boolean}
   */
  isBiznum(idttNo, separator) {
    const sep = separator ? separator : "";
    if (this.isEmpty(idttNo)) {
      return false;
    }
    if (idttNo.replace(new RegExp(sep, 'g'), '').length != 10) {
      return false;
    }
    const regExp = new RegExp(`^([0-9]{3})${sep}([0-9]{2})${sep}([0-9]{5})$`);
    let biznum = "";
    if (idttNo == null) {
      return false;
    } else {
      biznum = idttNo.replace(regExp, "$1$2$3");
    }

    // TODO 체크로직 검증 필요 2014-10-15 4108000968
    // return true;

    let sum = parseInt(biznum.charAt(0));
    const num = [0, 3, 7, 1, 3, 7, 1, 3];
    for (let i = 1; i < 8; i++) sum += (parseInt(biznum.charAt(i), 10) * num[i]) % 10;
    sum += Math.floor(parseInt(parseInt(biznum.charAt(8), 10)) * 5 / 10);
    sum += (parseInt(biznum.charAt(8), 10) * 5) % 10 + parseInt(biznum.charAt(9), 10);
    return (sum % 10 == 0) ? true : false;
  },

  /**
   * 법인 등록번호 체크 - args[0] : 등록번호 , args[1] : 등록번호 구분자
   * 000000-0000000
   * @param {String} idttNo
   * @param {String} separator
   * @returns {Boolean}
   */
  isCorpnum(idttNo, separator) {
    const sep = separator ? separator : "";
    const regExp = new RegExp(`^([0-9]{6})${sep}([0-9]{7})$`);
    let corpnum = "";
    if (idttNo == null) {
      return false;
    } else {
      corpnum = idttNo.replace(regExp, "$1$2");
    }
    let sum = 0;
    const num = [1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2];
    const last = parseInt(corpnum.charAt(12));
    for (let i = 0; i < 12; i++) {
   ...
   */
   isGratRgstNo(rgstNo) {
    return (/^([0-9]{4})-([0-9]{5})$/).test(rgstNo) ? true : false;
  },

  /**
   * 주민번호 또는 외국인등록번호 또는 사업자등록번호 체크 - args[0] : 등록번호 , args[1] : 등록번호 구분자
   * @param {String} idttNo
   * @param {String} separator
   * @returns {Boolean}
   */
  isIdttNo(idttNo, separator) {
    return this.isJumin(idttNo, separator) || this.isForeign(idttNo, separator) || this.isBiznum(idttNo, separator);
  },
  ...
  juminToBirth(idttNo) {
    ...
    const yymmdd = idttNo.substring(0, 6);
    const gubun = parseInt(idttNo.substring(6, 7), 10);

    if (gubun == '1' || gubun == '2' || gubun == '5' || gubun == '6')
      str = '19' + yymmdd;
    else if (gubun == '3' || gubun == '4' || gubun == '7' || gubun == '8')
      str = '20' + yymmdd;
    else if (gubun == '9' || gubun == '0')
      str = '18' + yymmdd;
